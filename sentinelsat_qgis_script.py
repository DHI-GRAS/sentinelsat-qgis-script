...
@alg.input(
    type=str, name="END", label="End date (YYYYMMDD)", default="NOW", optional=True
)
@alg.input(
    type=alg.ENUM,
    name="SENTINEL",
...
@alg.input(
    type=int,
    name="CLOUD",
    label="Maximum cloud cover in percent",
    minValue=0,
    maxValue=100,
    default=0,
    optional=True,
...
@alg.input(
    type=str,
    name="UUID",
    label="Select products by UUID (comma-separated)",
    optional=True,
)
@alg.input(
    type=str,
...
@alg.input(
    type=bool, name="DOWNLOAD", label="Download all results of the query", default=False
)
@alg.input(
    type=bool,
    name="FOOTPRINTS",
...
@alg.input(
    type=alg.FILE_DEST,
    name="PATH",
    label="Set the path where the the files will be saved",
)
def downloadsentinel(instance, parameters, context, feedback, inputs):
    ...
    def _extent_to_wkt(extent_str):
        return "POLYGON(({0} {2},{1} {2},{1} {3},{0} {3},{0} {2}))".format(
            *extent_str.split(",")
        )
    ...
    kwargs = dict(
        ...
        instrument=[None, "MSI", "SAR-C SAR", "SLSTR", "OLCI", "SRAL"][
            parameters["INSTRUMENT"]
        ],
        producttype=[None, "SLC", "GRD", "OCN", "RAW", "S2MSI1C", "S2MSI2Ap"][
    ...
    def cli(
        ...
        if uuid is not None:
            ...
            for productid in uuid_list:
                try:
                    ...
                    if "Invalid key" in e.msg:
                        logger.error(
                            "No product with ID '%s' exists on server", productid
                        )
            ...
            products = api.query(
                date=(start, end), order_by=order_by, limit=limit, **search_kwargs
            )
